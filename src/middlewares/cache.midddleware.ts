...
export const MCache = (options: CacheOptions) => {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      // Kalau method masuk dalam daftar yang tidak boleh cache
      if (invalidateOnMethods.includes(req.method)) {
        return next();
      }

      // Kalau ada kondisi untuk skip cache
      if (skipCacheIf && skipCacheIf(req)) {
        return next();
      }

      // Buat key cache berdasarkan request
      const cacheKey = generateCacheKey(req, keyPrefix);

      // Cek apakah ada data cache di Redis
      ...
      // Override res.send untuk menyimpan hasil response ke cache
...
const generateCacheKey = (req: Request, prefix: string): string => {
  ...
  const userId = "anonymous"; // tergantung implementasi auth
